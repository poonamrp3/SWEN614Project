...
## TODO: input your username here - only lower case alphanumerical values allowed!
locals {
  Username_for_s3Buckets = "your-username"
}

# Celebrity Look A Like Resources
################################################################################

# create an IAM Role for the PostImages lambda function
resource "aws_iam_role" "post_images_lambda_role" {
  name = "post_images_lambda_role"
...
# create a policy for the role post_images_lambda - s3 full access
resource "aws_iam_role_policy_attachment" "post_images_lambda_policy_s3" {
  role      =  aws_iam_role.post_images_lambda_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonS3FullAccess"
}

# create a policy for role post_images_lambda - CloudWatchFullAccess
resource "aws_iam_role_policy_attachment" "post_images_lambda_policy_cloudwatch" {
  role      =  aws_iam_role.post_images_lambda_role.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchFullAccess"
}

# create an IAM Role for the PostImages lambda to be accessed by API Gateway
...
resource "aws_lambda_function" "post_images_lambda" {
  function_name = "TFpost_images_lambda"
  handler       = "post_images.lambda_handler"
  runtime       = "python3.9"
  filename      = "post_images_lambda_payload.zip"  # Path to Lambda function code ZIP file
  source_code_hash = data.archive_file.post_images_lambda_zip.output_base64sha256  
  role          = aws_iam_role.post_images_lambda_role.arn
  ...
  environment {
    variables = {
      USER_INFO_BUCKET = aws_s3_bucket.user_images_bucket.bucket
    }
  }
}

################################################################################
########################## Aakanksha's API Gateway #############################
...
resource "aws_api_gateway_integration" "upload_post_integration_request" {
  ...
  uri = aws_lambda_function.post_images_lambda.invoke_arn # URI pointing to your Lambda function ARN
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  function_name = aws_lambda_function.post_images_lambda.function_name
...
resource "aws_amplify_app" "UIapp" {
  ...
  repository =  "https://github.com/km7872/SWEN614Team5"
...
################################################################################
################## Athina and Katyani's Rekognition section ####################

resource "aws_s3_bucket" "user_images_bucket" {
  bucket = "${local.Username_for_s3Buckets}-user-images-team5" # Bucket for user images
}
...
resource "aws_s3_bucket" "celebrity_images_bucket" {
  bucket = "${local.Username_for_s3Buckets}-celebrity-images-team5" # Bucket for celebrity images
}
...
resource "aws_lambda_function" "face_matching_lambda" {
  ...
  role          = aws_iam_role.face_matching_lambda_role.arn # IAM role for the Lambda function
...
# IAM role for Lambda
resource "aws_iam_role" "face_matching_lambda_role" {
  name = "face_matching_lambda_role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
# IAM role policies
resource "aws_iam_role_policy" "face_matching_lambda_role" {
  name = "face_matching_lambda_role"
  role = aws_iam_role.face_matching_lambda_role.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
